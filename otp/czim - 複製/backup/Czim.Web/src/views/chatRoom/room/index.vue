    ...
    <el-header>
      ...
      <div class="right-panel">
        <div class="right-panel-search">
          <!-- 搜索框 -->
          <el-input
            v-model="searchList.chatRoomName"
            placeholder="聊天室"
            clearable
          ></el-input>
          <!-- 查询 -->
          ...
          <el-button
            ...
            @click="resetSearch"
            :disabled="searchList.chatRoomName.length < 1"
          ></el-button>
        </div>
      </div>
    </el-header>
    <el-main class="nopadding">
      <div class="kxTable-table" v-loading="loading">
        ...
        >
          ...
          <el-table-column prop="chatRoomName" label="聊天室" />
          <el-table-column prop="chatRoomImg" label="封面"
            ><template #default="scope"
              ><el-image :src="scope.row.chatRoomImg"></el-image></template
          ></el-table-column>
...
export default {
  name: "list",
  components: { saveDialog, saveMember, saveAddManager },
  data() {
    return {
      dialog: {
        save: false,
  ...
  methods: {
    ...
    async getData() {
      this.loading = true;
      const res = await this.$API.chatRoom.findList.post(this.searchList);
      console.log(res);
      this.loading = false;
      if (res.code !== 0) {
    ...
    async doDefault(row) {
      this.loading = true;
      const res = await this.$API.chatRoom.doDefault.post({
        id: row.id,
