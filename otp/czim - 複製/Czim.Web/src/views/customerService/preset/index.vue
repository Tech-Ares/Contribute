    ...
    <el-main class="nopadding">
      <div class="kxTable-table" v-loading="loading">
        ...
        >
          ...
          <el-table-column prop="vague" label="模糊查询" width="100" />
          <el-table-column prop="abbre" label="缩略" width="100" />
          <el-table-column
            prop="context"
            label="消息内容"
...
export default {
  ...
  methods: {
    ...
    async getData() {
      this.loading = true;
      const res = await this.$API.preset.findList.post(this.searchList);
      console.log(res, "预设消息列表");
      this.loading = false;
      if (res.code !== 0) {
    ...
    async table_del(row) {
      this.loading = true;
      const res = await this.$API.preset.isActive.post({
        id: row.id,
