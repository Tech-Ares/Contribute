    ...
    <el-main class="nopadding">
      <div class="kxTable-table" v-loading="loading">
        ...
        >
          ...
          <el-table-column prop="appLoginName" label="App登录名" />
          <el-table-column prop="nickName" label="客服昵称" />
          <el-table-column prop="avatar" label="客服头像">
            <template #default="scope"
              ><el-avatar :src="scope.row.avatar"></el-avatar
            ></template>
          </el-table-column>
          <el-table-column prop="manTime" label="服务人数" />
...
export default {
  name: "loginList",
  components: { saveDialog, saveMember },
  data() {
    return {
      dialog: {
        save: false,
  ...
  methods: {
    ...
    async getData() {
      this.loading = true;
      const res = await this.$API.customerService.findList.post(
        this.searchList
      );
      console.log(res);
      this.loading = false;
    ...
    async table_del(row) {
      this.loading = true;
      const res = await this.$API.customerService.deleteActive.post({
        id: row.id,
