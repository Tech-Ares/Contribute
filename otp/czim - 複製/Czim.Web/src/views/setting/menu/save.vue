			...
			<el-col :lg="14">
				...
				<el-form :model="form" :rules="rules" ref="dialogForm" label-width="80px" label-position="left">
					...
					<el-form-item label="菜单排序" prop="number">
						<el-input-number v-model="form.number" :min="0" :step="1" step-strictly clearable></el-input-number>
					</el-form-item>
					...
					<el-form-item label="功能控制" prop="functions">
						<el-checkbox v-for="item in form.functions" v-model="item.isSelect" :key="item.id">{{ item.title }}</el-checkbox>
					</el-form-item>
...
export default {
	...
	methods: {
		...
		async save() {
			this.loading = true
			const res = await this.$API.sysMenu.menuSave.post(this.form);
			this.loading = false
			if (res.code !== 0) {
				this.$message.warning(res.message);
				return;
			}
			this.serverMenuId = res.data
			this.$message.success("保存成功")
			this.$emit('on-save', this.serverMenuId);
		},
		//表单注入数据
		// setData(data, pid="") {
		// 	this.form = data
