...
if ! grep -q '@@@@@' "${PID_FILE}"; then
	echo "kill 進程"
    ps -ef |grep gateway-rbac.conf | grep -v grep |awk '{print $2}'| xargs kill -9
    ps -ef |grep rbacGatewayNode1 | grep -v grep |awk '{print $2}'| xargs kill -9
	echo '@@@@@' > ${PID_FILE}
	sleep 10s
	su - iceuser -c 'nohup icegridnode --Ice.Config=/home/version3.0/iceconfig/gateway-rbac.conf 2>&1 &'
	echo -e "正在檢查服務是否正常啟動. . . . ."
	for i in {1..4}; do
		sleep 3s
